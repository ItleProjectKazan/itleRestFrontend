...
import { Button, Layout, Loader, ProductsSlider, Radiobox, Section, Tooltip, Typography } from '~/components'
...
import { parseRestaurantCookie, addYm } from '~/helpers'
import { ApiEndpoints } from '~/constants/apiEndpoints'
import { TProduct } from '~/types/catalog'
import { TCartItem } from '~/types/cart'
import { TOrderStep, TOrderType } from '~/types/order'
...
const Cart: NextPage<Props> = observer(({ recommendedProducts }) => {
    ...
    const cart = useCart()
    const { loginModal, user, orderParams, restaurantSelectModal } = useStore()
    ...
    return (
        <Layout
            classes={{ footer: 'd-none-phone-large', main: styles.main }}
            orderStep={TOrderStep.CART}
            recommendedProducts={recommendedProducts}
            title='Корзина'
        >
            <Link className={styles.closeIcon} href={PageLinks.HOME}>
                <CloseIcon />
            </Link>
